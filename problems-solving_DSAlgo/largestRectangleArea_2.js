// 84. Largest Rectangle in Histogram
// Hard
// Topics
// premium lock icon
// Companies
// Given an array of integers heights representing the histogram's bar height where the width of each bar is 1, return the area of the largest rectangle in the histogram.

// Example 1:

// Input: heights = [2,1,5,6,2,3]
// Output: 10
// Explanation: The above is a histogram where width of each bar is 1.
// The largest rectangle is shown in the red area, which has an area = 10 units.
// Example 2:

// Input: heights = [2,4]
// Output: 4

// Constraints:

// 1 <= heights.length <= 105
// 0 <= heights[i] <= 104

/**
 * @param {number[]} heights
 * @return {number}
 */
// var largestRectangleArea = function (heights) {
//   let result = 0;
//   const stack = [];
//   let runningMax = 0;
//   for (let i = 0; i < heights.length; i++) {
//     const current = heights[i];
//     let topOfStack = stack.length - 1;
//     while (
//       topOfStack >= 0 &&
//       stack[topOfStack] !== undefined &&
//       stack[topOfStack].value > current
//     ) {
//       runningMax = current * (i - stack[topOfStack].index + 1);
//       result = Math.max(runningMax, result);
//       stack.pop();
//       topOfStack--;
//     }
//     stack.push({ value: current, index: i });
//     topOfStack = stack.length - 1;
//     while (topOfStack >= 0) {
//       runningMax = stack[topOfStack].value * (i - stack[topOfStack].index + 1);
//       result = Math.max(result, runningMax);
//       topOfStack--;
//     }
//   }

//   return result;
// };

var largestRectangleArea = function (heights) {
  const stack = []; //stores indexes
  let maxArea = 0;
  heights.push(0);
  for (let i = 0; i < heights.length; i++) {
    let currentHeight = heights[i];
    while (stack.length && currentHeight < heights[stack[stack.length - 1]]) {
      const topHeight = stack.pop();
      const height = heights[topHeight];
      const width = i - topHeight + 1;
      maxArea = Math.max(maxArea, width * height);
    }
    stack.push(i);
  }
  return maxArea;
};

console.log(largestRectangleArea([2, 1, 5, 6, 2, 3]));
console.log(largestRectangleArea([2, 4]));
console.log(largestRectangleArea([2, 1, 2]));
